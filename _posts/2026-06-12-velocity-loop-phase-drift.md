...
说限制之前，先拆一个极易混淆的概念。写控制代码的人熟悉的是这组数：策略 50 Hz、单片机 200 Hz、驱动器 1000 Hz——这些是**更新率**（Update Rate），即控制器每隔多久重新计算一次输出，写在代码里、自己定的软件节奏。而**带宽**是另一回事：闭环系统实际能跟上多快变化的命令。给速度环一个正弦速度命令，频率低时实际转速跟得上；频率高过某个点，实际转速的摆幅明显衰减、相位滞后——那个点才是带宽。它由惯量、转矩上限、滤波和增益共同决定，不归你定。

类比：更新率是**你多久看一次仪表盘**，带宽是**这辆车加速有多快**。每毫秒看一次速度表，车从 0 到 100 还是要 5 秒——看表再勤快，发动机扭矩不会变大。

一阶近似下，带宽和阶跃响应的时间常数（Time Constant）互为倒数：

$$f_{bw} \approx \frac{1}{2\pi\tau}$$

符号含义：$f_{bw}$ 是闭环带宽（Hz），$\tau$ 是时间常数（s），即阶跃命令下实际响应爬到目标 63% 所需的时间。

> **代入数字**：速度环对阶跃速度命令的响应时间常数 $\tau = 40\ \mathrm{ms}$：
> $$f_{bw} = \frac{1}{2\pi \times 0.04} \approx 4\ \mathrm{Hz}$$
> 驱动器明明每 1 ms 算一次（更新率 1000 Hz），带宽却只有 4 Hz——因为转子有惯量、转矩有上限，算得再勤，转速物理上就是不能突变。更新率只是带宽的天花板之一（经验上更新率要比自身带宽高 10~20 倍），但它不是带宽本身。

放到一个典型的机器人控制栈里，每一层都同时有这两个数：

| 层 | 更新率（软件定） | 闭环带宽（物理定，典型量级） | 带宽卡在哪 |
|------|------|------|------|
| 电流环 | 1~20 kHz | 100 Hz ~ 1 kHz | 绕组电感与电阻 |
| 速度环 | 200 Hz ~ 1 kHz | 几 Hz ~ 几十 Hz | 转子+负载惯量、转矩上限 |
| 相位环/策略 | 50 Hz | 约 1~2 Hz | 整机动力学、接触 |

下面说的"外环带宽被内环封顶"，比较的是各层的**带宽**，跟更新率无关——只要每层的更新率足够高于自己的带宽就行。

### 限制一：外环带宽被内环封顶

级联控制有条经验法则：外环带宽要比内环慢 3~5 倍，否则两个环互相打架、系统振荡。物理含义是**外环要有耐心**：内环执行一条命令的特征时间约为 $1/(2\pi f_{\text{内}})$——4 Hz 带宽对应约 40 ms。外环如果在内环还没执行完上一条命令时就因为"误差还没消"而继续加码，修正叠着修正，两个环互相追逐，就振荡了。外环必须等内环把上一条执行得差不多了再下结论。
...
| 时间常数 | Time Constant | 阶跃响应爬到目标值 63% 所需的时间，带宽的倒数量级 |
| 更新率 | Update Rate | 控制器重新计算输出的频率，由软件决定，不等于带宽 |
